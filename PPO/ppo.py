...
LastEditors: JIANG Bozhen
LastEditTime: 2024-10-23 16:32:54
'''
import numpy as np
import os
import logging
...
class PPO:
    ...
    def __init__(self, act_dim, env_dim, gen_num, bus_num, args):
        ...
        # 示例：模拟训练过程记录日志
        logging.info('Start PPO instantiation')
    ...
    def policy_action(self, s):
        ...
        if self.noise:
            ...
            # _action = tf.clip_by_value(mu, clip_value_min=0, clip_value_max=1)
            ...
            mu, sigma = self.actor.predict(s)
            _action = tf.clip_by_value(tf.random.normal([1], mu, sigma, tf.float32), clip_value_min=0, clip_value_max=1).numpy()
            # _action = tf.clip_by_value(mu, clip_value_min=0, clip_value_max=1).numpy()
            return _action
    ...
    def compute_advantage(self, td_delta):
        ...
        return tf.convert_to_tensor(advantage_list)
        ...
        for e_,episode in enumerate(tqdm_e):
            ...
            for timestep in range(24):
                ...
                if info == "True" :
                    ...
                    #更新环境
                    env = new_env
                    next_state = self.normal_state(env,
                                                    flows, 
                                                    timestep, 
                                                    normal=self.normal)   
                ...
                #     #更新环境
                #     env = new_env
                #     next_state = _julia_data_to_python_data(env,flows,
                #                                             timestep,
                #                                             self.expand_states)
                ...
                #     break
                    ...
                    logging.info(f'Timestep {timestep} : Agent solution randomly save')
                    solver_reward += r
                    #记录Agent真实的回报
                    if r_true_flag == 0:
                        r_true += r_agent 
            ...
            if actions.size != 0:
                ...
                target_Q = self.critic.predict(next_states)
                td_target = self.bellman(rewards, target_Q, dones)

                td_delta = td_target - self.critic.predict(states)
                advantages = self.compute_advantage(td_delta)

                mu,std = self.actor.predict(states)
                # print(mu)
                # print(std)
                new_action_dists = tfp.distributions.Normal(mu, std)
                old_log_probs = new_action_dists.log_prob(actions)
                _actor_loss = 0
                _critic_loss = 0
                for _ in range(5):
                    _actor_loss += self.actor.train(states, actions, old_log_probs, advantages)
                    _critic_loss += self.critic.train(states, td_target)

                logging.info(f'Episode {episode} : agent_reward {str(round(agent_reward, 2))}, solver_reward {str(round(solver_reward, 2))},not_legal_times {not_legal_times},_actor_loss  {str(round(float(_actor_loss)/5, 2))},_critic_loss {str(round(float(_critic_loss)/5, 2))}')        
                ...
                tqdm_e.set_description("Score: " + str(agent_reward)+" "+
                                    ...
                                    "First Wrong Timestamp: " +str(first_wrong_timestamp+1) + " "+
                                    "True Score: " + str(r_true)+" "+
                                    "Agent Fail Times: " + str(not_legal_times)+" "+
                                    "Agent Actor Loss: " + str(round(float(_actor_loss)/5, 5))+" "+
                                    "Agent Critic Loss: " + str(round(float(_critic_loss)/5, 5)))
                tqdm_e.refresh()  
