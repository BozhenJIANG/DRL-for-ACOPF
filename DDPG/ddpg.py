...
class DDPG:
    ...
    def __init__(self, act_dim, env_dim, gen_num, bus_num, args):
        ...
        # 示例：模拟训练过程记录日志
        logging.info('Start DDPG instantiation')
    ...
    def train(self, summary_writer):
        ...
        for e_,episode in enumerate(tqdm_e):
            ...
            _critic_loss = 0
            _actor_loss = 0
            # Defaults so that episode-end stats never reference undefined vars
            # on the first episodes, before the buffer holds enough experience
            # for the training blocks to run.
            update_times = 1
            ...
            for timestep in range(24):
                ...
                if info == "True" :
                    ...
                    if _info == "False":  
                        ...
                        _new_env, (_, _r), _done, _info = self.jl.actor_solve_and_step(env,
                                                        self.train_dataset[sample_episode,timestep,self._non_zero_index][0],
                                                        self.train_dataset[sample_episode,timestep+24,self._non_zero_index][0],
                                                        action,
                                                        _env) 
                        
                        new_env, a, (_, r), done, info = self.jl.opf_solve_and_step(env,
                                                    self.train_dataset[sample_episode,timestep,self._non_zero_index][0],
                                                    self.train_dataset[sample_episode,timestep+24,self._non_zero_index][0],
                                                    _env)
                        break
                ...
                if self.buffer.count > 1:
                    ...
                    if (timestep+1) % (2*self.freq) == 0 :
                        ...
                        for index_ in range(update_times):
                            ...
                            target_Q = self.critic_target.predict([new_states, 
                                                                next_action])
            ...
            if self.buffer.count > 1:
                ...
                if (self.load_weight_episode+episode+1) % 200 == 0: 
                    if self.load_weight_episode+episode+1 == 200:
                        ...
                        pre_reward_list = np.load("./"+self.case_name+"/"+
                                                ...
                                                str(self.lr)[2:]+"_"+
                                                str(int(self.expand_states))+"_"+
                                                str(int(self.noise))+"_"+str(int(self.normal))+".npy",
                                                allow_pickle=True)
                        ...
                        np.save("./"+self.case_name+"/"+
                                ...
                                str(int(self.expand_states))+"_"+
                                str(int(self.noise))+"_"+
                                str(int(self.normal))+".npy",reward_list)
